...
import cc.redberry.rings.legs.{CachedIntegralVal, IntegralSignature, IntegralsDB}
...
class MasslessIntegrals[E](cfRing: Ring[E],
  ...
  val polyRing = MultivariateRing(cfRing, Array("d", "p12", "p23", "p34", "p45", "p15", "p13", "p14", "p24", "p25", "p35"))
  ...
  /** retrieves integral value from db or computes it from lambda */
  private
  def getOrCompute(integral: IntegralDef[Expr],
                   compute: () => IntegralVal[Expr]): IntegralVal[Expr]
  = database match {
    case Some(db) if integral.indices.product != 0 =>
      val signature: IntegralSignature[Expr] = integralSignature(integral)
      val cachedIntegral: CachedIntegralVal[Expr] = db.get(signature, ring)
      if (cachedIntegral != null) {
        log("Using cached value for " + integral.stringify)
        cachedIntegral.iVal.map(r => Util.replaceVariables(r, cachedIntegral.iDef.args, integral.args))
      } else {
        log("Computing " + integral.stringify)
        val result = compute()
        db.put(signature, CachedIntegralVal(integral, result), ring)
        result
      }
    case _ =>
      log("Computing " + integral.stringify)
  ...
  def I5(n1: Int, n2: Int, n3: Int, n4: Int, n5: Int, d: Expr,
    ...
    getOrCompute(iDef, () => __I5(iDef, n1, n2, n3, n4, n5, d, s12, s23, s34, s45, s15, s13, s14, s24, s25, s35))
  }
  ...
  def I4(n1: Int, n2: Int, n3: Int, n4: Int, d: Expr,
    ...
    // get from cache or compute from scratch
    getOrCompute(iDef, () => __I4(iDef, n1, n2, n3, n4, d, s12, s23, s34, s14, s24, s13))
  }
  ...
  def I3(n1: Int, n2: Int, n3: Int, d: Expr, s23: Expr, s13: Expr, s12: Expr): IntegralVal[Expr] = {
    // integral definition
    val iDef = IntegralDef(threePointDef, Seq(n1, n2, n3), Seq(d, s23, s13, s12))
    // get from cache or compute from scratch
    getOrCompute(iDef, () => __I3(iDef, n1, n2, n3, d, s23, s13, s12))
  }
